...
## Ajay Pipes Context — read before running any check

1. `on-page-seo-ajaypipes.md` — **verified live-site findings.** FAQPage and Product schema are the real gaps on ajaypipes.com; Organization/WebSite/WebPage/Article/BreadcrumbList/Person already exist sitewide via Yoast. Don't fail a check for "missing Organization schema" — check for FAQPage instead.
...
| **Product page linked** | **(Ajay-specific, required)** At least one internal link goes to a specific Ajay product page (Flowline Plus, Greenline, Drainline, Terraline, Agriline, or Deepline) — links only to other blog posts is a FAIL on this site |
...
### Step 6.5: Ajay Voice & Taboo Check

| Check | Pass Criteria |
|-------|---------------|
| No competitor names | Zero mentions of Supreme, Astral, Finolex, Ashirvad, Apollo, Truflo, Flowguard, Prayag, or any other competitor |
| No underdog framing | Zero defensive phrasing ("even though," "unlike bigger brands," "catching up to") |
| Taboo phrases | Zero matches against `BRAND.md`'s taboo list (unlock, leverage, seamless, world-class, best-in-class, comprehensive solution, "in today's fast-paced world," "we pride ourselves on," "contact us" as sole CTA) |
| Numbers verified | Every Ajay-specific number matches `references/stats.md` exactly |
| Humour register | Dry on product/technical pages; plain-spoken warmth only (no jokes/puns) on homeowner-facing posts — see `references/humour.md` |
