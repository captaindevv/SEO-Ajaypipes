...
Ajay-specific numbers (years in business, certifications, test parameters, product specs) are **already verified against live company sources** in `references/stats.md` — that file is the canonical ground truth for this brand, not a claim to re-fact-check against arbitrary web results. Skip these in the external-verification workflow below; instead just confirm the figure in the post matches `stats.md` exactly (a simple string/value comparison, not a WebFetch verification). Only external statistics (industry data, third-party studies) go through the full Steps 1-5 verification process.
