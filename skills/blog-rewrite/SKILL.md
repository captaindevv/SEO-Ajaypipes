...
This deployment rewrites posts for ajaypipes.com. Read every file below before Phase 1 — they override the engine's generic defaults wherever the two conflict.
...
4. `on-page-seo-ajaypipes.md` — **verified live-site findings, don't rediscover from scratch**: FAQPage + Product schema are the confirmed real gaps (Organization/Article/Breadcrumb/Person already exist via Yoast — don't flag those as missing); sitemap is stale and excludes all 2025/2026 posts (a platform issue, not something this rewrite can fix); `/cpvc-full-form/` and `/cpvc-pipe-full-form/` cannibalization is confirmed still live
5. `references/voice.md` — full sentence rhythm, banned words, "tells it's AI-written" checklist
...
2. **Run the quality checklist** against `references/quality-scoring.md`:
   ...
   - Look for FAQ schema — **on ajaypipes.com, FAQPage is the confirmed real gap; don't waste the audit flagging Organization/Article/Breadcrumb/Person as missing, they already exist via Yoast (see `on-page-seo-ajaypipes.md`)**
   - Check freshness signals (lastUpdated, dateModified)
   - **Check for at least one internal link to a specific Ajay product page** (not just other blog posts) — this site's top-traffic posts currently link to zero product pages; flag this explicitly if absent
...
5. **Cannibalization check**:
   ...
   - If cannibalization found, report:
     ...
     - For the 3 documented clusters specifically: default recommendation is consolidation into one authoritative post per cluster, not further optimization of all of them individually
...
- **Ajay-specific**: "maintain internal links" is not sufficient if none of them point to a product page. If the original post links only to other blog posts, this rewrite must add at least one link to the relevant Ajay product page (Flowline Plus, Greenline, Drainline, Terraline, Agriline, or Deepline) with descriptive anchor text — see `on-page-seo-ajaypipes.md`.
...
- **Author byline**: the current site uses a generic "ajaypipes" byline with no real author signal (confirmed on the live top-traffic post). If a real named technical staff member has been supplied, use them. Otherwise default to an organizational byline ("Ajay Pipes Technical Team") — don't invent an individual's name/credentials. Flag the missing real-author gap to the user; it's a genuine E-E-A-T weakness worth fixing site-wide, not just per-post.
...
- **This is the confirmed real schema gap on ajaypipes.com** — verified directly that FAQPage JSON-LD is absent sitewide even on posts with visible FAQ content. Don't skip this step assuming Yoast already covers it; it covers Article/Breadcrumb/Person/Organization, not FAQPage. Generate via `/blog schema`.
...
#### 4o. Apply Ajay Voice & Humour Register
- Re-read `references/voice.md` and `references/humour.md` before touching prose.
- Determine the correct register per `humour.md`: dry-professional (product/technical/commercial pages) vs plain-spoken warmth (homeowner-facing educational posts) — never jokes or puns in either register.
- Rewrite any sentence that only makes sense next to a competitor's name or implies Ajay is behind/smaller/catching up (see `BRAND.md` taboo list) — replace with a fact from `references/stats.md` that stands on its own.
- Replace vague superlatives ("high quality," "trusted," "reliable") with the specific certification/spec/number that earns the claim.
- If a story or opinion fits naturally, pull one from `references/stories.md` / `references/opinions.md` (verbatim-adapted, never invented) — max one of each.
...
#### Ajay-Specific Gates (in addition to the above)
20. **No underdog framing** - Zero competitor names. Zero defensive phrasing. Every authority claim traces to `references/stats.md`.
21. **Taboo phrases removed** - Checked against `BRAND.md`'s taboo list.
22. **Humour register correct** - Dry on product/technical pages, plain-spoken warmth only (no jokes/puns) on homeowner-facing posts.
23. **At least one internal link to a specific Ajay product page** - not just other blog posts.
24. **FAQPage schema added** - the confirmed real gap, not Organization/Article/Breadcrumb/Person (those already exist).
25. **Numbers match `references/stats.md` exactly** - no rounding, no invented figures.
26. **Cannibalization cluster checked** - if this post is in one of the 3 documented clusters in `references/used-keywords.md`, the summary says so and recommends consolidation, not just this post's isolated optimization.
