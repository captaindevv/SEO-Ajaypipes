...
## Ajay Pipes Context — read before scoring

1. **Schema scoring depends on whether the target is a local draft or a live ajaypipes.com URL.** ajaypipes.com runs WordPress + Yoast, which auto-injects `Organization`/`WebSite`/`WebPage`/`Article`/`BreadcrumbList`/`Person` schema at render time (confirmed on live pages; see `on-page-seo-ajaypipes.md`). A local draft file will correctly show these as absent (they don't exist yet, Yoast adds them on publish) — don't penalize a pre-publish draft for missing Organization/Article/Person schema. When scoring a **live URL**, those types will already be present via Yoast; the only schema type genuinely worth scoring as missing is `FAQPage` (confirmed absent sitewide even where FAQ content exists) and `Product` (product pages only).
