...
- **Link TO** (from this new post to existing pages):
  0. **(Ajay-specific, required)** [Specific product page: Flowline Plus / Greenline / Drainline / Terraline / Agriline / Deepline] - anchor text: "[descriptive anchor]"
...
- **Experience**: [First-hand insight, case study, or test result — pull from `references/stories.md` if a topical match exists, never invent]
- **Expertise**: [Author credentials relevant to topic — use a real named technical staff member if supplied, otherwise "Ajay Pipes Technical Team" as an organizational byline, never a fabricated individual]
- **Authority**: [From `references/stats.md`: 65+ years, oldest ISI-licensed UPVC pipe maker in India, relevant certifications for this product line]
- **Trust**: [Transparency, sourced data, no self-promotion, no competitor comparisons — see `BRAND.md`]
