...
`BRAND.md`'s taboo list and anti-underdog rule apply on every platform below, not just the blog — social/community formats are actually higher-risk for this brand because their native conventions (hot takes, "here's what beats X," peer comparison) invite exactly the competitor-naming and defensive framing this brand must avoid. No competitor names in any repurposed output, ever.

**LinkedIn's "first-person perspective" convention (Step 4) doesn't map cleanly to this brand** — there's no individual named spokesperson by default (see `VOICE.md`). Default to the organizational "we" voice adapted to LinkedIn's shorter, punchier formatting, unless a real named person has been designated to post under their own name.
