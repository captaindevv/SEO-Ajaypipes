...
1. **Competitors are already known — don't re-ask Step 1's "who are your competitors."** Big 4 (out of reach short-term): Supreme, Astral, Finolex, Ashirvad. Realistic peer group (Ajay already leads on fundamentals): Apollo, Truflo, Flowguard, Prayag. **Xtruline and XtrulinePower are NOT competitors** — they're the company director's separate businesses, entirely out of scope (see `BRAND.md` topic scope).
2. **Critical: competitive research in this skill is internal strategy analysis only.** The Competitive AI Citation Map (Step 2), Competitive Positioning section (Step 8 output), and any "how we differentiate vs X" framing must stay in the strategy document — **never carry a competitor name or direct comparison into a content brief, outline, or published post.** See `BRAND.md` Editorial Rules: no competitor names, no underdog framing, ever, in reader-facing content. The strategy doc can say "Astral ranks #1 for X, Ajay doesn't rank" as an internal gap-analysis note; the resulting blog post about X must not mention Astral.
3. `references/used-keywords.md` — Step 4's content pillar design must check the 3 documented cannibalization clusters before proposing new pillars. ~25 of the 100 existing posts already sit in "best/top pipe company/brand," "best pipe for plumbing," or "which pipe is best" — a new pillar built around the same intent compounds the problem rather than fixing it.
4. `BRAND.md` / `references/stats.md` — audience, positioning, and differentiation signals (Step 5) should be grounded in Ajay's real facts (65+ years, oldest ISI-licensed UPVC maker, 101+ test parameters, NSF/ANSI certification) rather than generic "build authority" language.
...
[How we differentiate - what unique value we bring, grounded in `references/stats.md` facts]
[Competitive AI Citation Map showing gaps to exploit]
[Internal-only section — do not carry competitor names or direct comparisons into any brief/outline/post generated from this strategy]
