...
8. `references/stories.md` — real anecdotes only, one per post max, never invented. If empty or nothing topical fits, write the post without a story rather than inventing one
9. `references/opinions.md` — real technical stances only, one per post max, backed by a `stats.md` number. Same rule: skip rather than invent
10. `references/used-keywords.md` — **check before picking any primary keyword.** Three semantic-cannibalization clusters already exist covering ~25 of the 100 published posts ("best/top pipe company/brand," "best pipe for plumbing," "which pipe is best" — full lists in the file). Do not propose a topic that adds to one of these clusters; if the requested topic overlaps, say so and propose consolidating the existing posts instead of writing a new one.
