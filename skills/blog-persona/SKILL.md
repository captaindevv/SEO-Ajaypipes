...
`VOICE.md` already has a qualitative fingerprint for this brand (see its "Voice fingerprint" section) — strongly serious, mid-formal, fully respectful, matter-of-fact. When running `/blog persona create` for this project, pre-fill Step 2's sliders from that description instead of starting blind, and confirm with the user rather than re-deriving from scratch. Whatever persona is created must stay consistent with `BRAND.md`'s non-negotiable rules (no underdog framing, no competitor names) — a persona cannot override those.
