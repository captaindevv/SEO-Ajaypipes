...
## Ajay Pipes Context — critical, read before generating anything

ajaypipes.com runs WordPress + Yoast SEO, which **already auto-generates its own schema graph on every page** — confirmed directly (checked raw HTML): `Organization`, `WebSite`, `WebPage`, `BreadcrumbList`, `Article`, `Person` (author) all already exist sitewide. Full detail in `on-page-seo-ajaypipes.md`.

**Do not regenerate BlogPosting/Article, Organization, BreadcrumbList, or Person schema for Ajay content — Yoast already emits these, and a second competing graph on the same page is a real problem (duplicate/conflicting entities confuse Google, not a neutral no-op).**
...
**For ajaypipes.com specifically**, exclude `BlogPosting`, `Person`, `Organization`, and `BreadcrumbList` from the output — Yoast already generates those (see Ajay Pipes Context above). The Ajay output graph is just:
...
For any other project without an existing Yoast/CMS-generated graph, use the full 7-type pattern below as the default.
