...
## Ajay Pipes Context — read before auditing

1. `on-page-seo-ajaypipes.md` — **`llms.txt` does not exist on ajaypipes.com (confirmed 404)** and the sitemap is stale, missing all 2025/2026 content. Both directly affect Step 6 (AI Crawler Accessibility) and freshness scoring — report them as the root cause rather than treating each post as independently under-optimized. FAQPage schema is also confirmed absent (relevant to Step 5/6 schema checks) even though Article/Organization/etc. exist via Yoast — don't flag those as missing.
2. `references/stats.md` — any citation capsule generated in Step 8 must use only numbers from this file, never rounded or invented.
3. `BRAND.md` — citation capsules must not name competitors or use comparison-based framing, even though that's a common AI-citation pattern elsewhere. Confidence in capsules comes from Ajay's own verified facts (65+ years, oldest ISI-licensed UPVC maker, 101+ test parameters).
4. `references/used-keywords.md` — note if the post under audit is in one of the 3 documented cannibalization clusters; a post with divided topical authority across ~14 near-duplicate posts will structurally underperform on Entity Clarity (Step 4) regardless of on-page fixes.
